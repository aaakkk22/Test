$(function() {
	...
	window.Manager = {
		...
		init : function(){
			...
			$('#upload_tab,#manage_tab,#search_tab,#icon_tab').click(function(){
		...
		showList : function(){
			if(this.lock1 == true) return;
			this.lock1 = true;
			this.showFiles({type : this.opts.type}, $('#file_all_list'))
		},

		showIconList: function(){
			if(this.lock3 == true) return;
			this.lock3 = true;
			this.showIconFiles({type : this.opts.type}, $('#file_all_icon_list'))
		},


		showSearch : function(key){
			if(this.lock2 == true) return;
			this.lock2 = true;
			var data = {};
			if(key) data.key = key;
		...
		//显示图标
		showIconFiles: function(data, $container){
			var __self = this;
			$container.empty().parents('div.area').addClass('loading');
			data = typeof data == 'object' && data != null ? data : {};
			data = $.extend(data, this.opts.formData);
			$.ajax({
				url : __self.opts.filelistPah+'?icon=1',
				data : data,
				dataType : 'json',
				success : function(data){
					var $html = __self.createFile(data);
